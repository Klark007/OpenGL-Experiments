// based of https://github.com/sevanetrebchenko/fast-poisson-disk-sampling/tree/master adapted to use glm vector types
...
namespace fpds {
    ...
    struct grid {
        grid(const glm::vec2& dimensions, float separation_distance)
        ...
        [[nodiscard]] glm::ivec2 convert_to_grid_coordinates(const glm::vec2& world_coordinates) const {
    ...
    [[nodiscard]] std::vector<glm::vec2> fast_poisson_disk_2d(glm::vec2 dimensions, float r, int k = 30) {
        grid g{ dimensions, r };
        ...
        glm::vec2 sample_world_coordinates = glm::vec2(uniform_real_distribution(0.0f, dimensions.x),
            uniform_real_distribution(0.0f, dimensions.y));

        // Record sample in grid.
        int sample_index = 0;
        glm::ivec2 sample_grid_coordinates = g.convert_to_grid_coordinates(sample_world_coordinates);
        ...
        while (!active_list.empty()) {
            ...
            for (int i = 0; i < k; ++i) {
                ...
                glm::vec2 test_sample_world_coordinates = glm::vec2(sample_world_coordinates.x + radius * cosf(radians),
                ...
                glm::ivec2 test_sample_grid_coordinates = g.convert_to_grid_coordinates(test_sample_world_coordinates);
                ...
                for (int y = -1; y < 2; ++y) {
                    for (int x = -1; x < 2; ++x) {
                        ...
                        if (test_sample_index != NO_SAMPLE) {
                            ...
                            const glm::vec2& existing_sample = point_list[test_sample_index];

                            // Ensure separation between current and test point is at least 'r'.
                            if (glm::distance(existing_sample, test_sample_world_coordinates) < r) {
                                valid_sample = false;
                                break;
                            }
                        }
